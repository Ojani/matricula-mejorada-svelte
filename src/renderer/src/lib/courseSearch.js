...
// Searches for course sections using the course code, year, and term

export default async function searchSections({ COURSE_CODE, TERM, YEAR }) {
  // Defaulting year to the current one
  if (!YEAR) YEAR = new Date().getFullYear()

  const searchQuery = `https://www.uprm.edu/registrar/sections/index.php?v1=${encodeURI(COURSE_CODE)}&op2=1&v2=&term=${encodeURI(TERM)}-${encodeURI(YEAR)}&a=s&cmd1=Search`
